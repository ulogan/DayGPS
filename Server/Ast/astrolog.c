/*
** Astrolog (Version 5.05) File: astrolog.c
**
...
void Action()
{
  ...
  szAspectAbbrev[aCon] = us.fParallel ? "Par" : "1st";
  szAspectAbbrev[aOpp] = us.fParallel ? "CPr" : "7th";
...
char **argv;
{
  ...
  case 'A':
    ...
    lpr = ch1 == 'm' ? rObjOrb : (ch1 == 'd' ? rObjAdd : rAspOrb);
    for (k = i; k <= j; k++)
...
char **argv;
{
  ...
  while (argc) {
    ...
    case 'h':
      ...
      us.objCenter = i;
      pch = szObjName[0];
      szObjName[0] = szObjName[us.objCenter];
      szObjName[us.objCenter] = pch;
      if (us.objCenter < oMoo)
        us.objCenter = 1-us.objCenter;
      break;
    ...
    case 'r':
      ...
      if (ch1 == 'c')
        ...
        us.nRel = rcSynastry;
      is.szFile = SzPersist(argv[1]); is.szFile2 = SzPersist(argv[2]);
      if (is.fSzInteract) {
        if (!FInputData(is.szFile2))
          return fFalse;
        ciTwin = ciCore;
        if (!FInputData(is.szFile))
          return fFalse;
      }
      if (i > 2) {
    ...
    case 'y':
      ...
      if (ch1 == 'd')
        ...
        us.nRel = rcDual;
      is.szFile = SzPersist(argv[1]); is.szFile2 = szNowCore;
      argc--; argv++;
      break;
#endif
...
LBegin:
  ...
  if (us.fLoop) {       /* If -Q in effect, loop back and get switch */
