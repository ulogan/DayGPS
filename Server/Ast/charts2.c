/*
** Astrolog (Version 5.05) File: charts2.c
**
      ...
      for (j = 1; j <= cObj; j++) if (!ignore[j])
        if (k = grid->n[i][j]) {
          ...
          p = (int)(rAspInf[k]*(ip+jp)/2.0*
            (1.0-RAbs((real)(grid->v[i][j]))/60.0/rAspOrb[k])*1000.0);
...
#ifdef WIN
  if (is.szFile == NULL || is.szFile2 == NULL)
    fFile = fFalse;
#endif
  if (fFile)
...
#else /* notdef */
  if (aspect != -13 && aspect != -15 && aspect != -17 && source != 21 && dest != 21) {
    printf("<->");
  } else {
    printf("   ");
  }
...
char chart;
{
  ...
  if (chart == 't' || chart == 'T')
    ...
    PrintSz("progr ");
  if (asp == -13) {
    sprintf(sz, "%-14s", naksatras[sign1]); PrintSz(sz);
  } else if (asp == -15) {
    sprintf(sz, "%-18s", tithis[sign1]); PrintSz(sz);
  } else if (asp == -17) {
    sprintf(sz, "%-14s", yogas[sign1]); PrintSz(sz);
  } else {
    if (sunraise) {
        if (obj2 == oAsc) {
            sprintf(sz, "%-8s", szObjName[obj1]); PrintSz(sz);
        }
    } else {
  ...
  if (sunraise) {
    ...
    sprintf(sz, "-->");                        /* Print a sign change. */
     ...
     sprintf(sz, "**>");                        /* Print a Navamsa change. */
  else if (asp == -13) {
     sprintf(sz, "");                        /* Print a Naksatra change. */
  } else if (asp == -15) {
     sprintf(sz, "");                        /* Print a Tithi change. */
  } else if (asp == -17) {
     sprintf(sz, "");                        /* Print a Yoga change. */
  } else if (asp == aDir)
    sprintf(sz, "S/%-16c", obj2 ? chRet : 'D');   /* Print a direction retrogration change. */
  else if (asp == -9)
    sprintf(sz, "S/%-15c", obj2 ? 'F' : 'D');   /* Print a direction progression change. */
  else if (asp == 0)
  ...
  if (sunraise) {
      //
  } else if ((asp == aSig) || (asp == -11)) {
    AnsiColor(kSignA(obj2));
    sprintf(sz, "%-14s", szSignName[obj2]); PrintSz(sz);
  } else if (asp == -13) {
    // sprintf(sz, "%-14s", naksatras[obj2]); PrintSz(sz);
  } else if (asp == -15) {
    // sprintf(sz, "%-14s", tithis[obj2]); PrintSz(sz);
  } else if (asp == -17) {
    // sprintf(sz, "%-14s", yogas[obj2]); PrintSz(sz);
  } else if (asp >= 0) {
    ...
    sprintf(sz, "%-8s", szObjName[obj2]); PrintSz(sz);
  }
#ifdef notdef /* Logan */
